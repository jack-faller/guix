...
** Case Change
#+begin_src emacs-lisp
  (defun capitalise-first (string)
    (string-join (list (upcase (substring string 0 1)) (downcase (substring string 1)))))
  (defun case-convert (ident from to)
    (let* (case-fold-search
           ...
           (matches
            (save-match-data
              (cl-loop
               for j = (string-match regexp ident) then (string-match regexp ident (match-end 0))
               while j
               do (message "%s" j)
               collect (match-string 0 ident)))))
      (pcase to
        ('upper-snake (string-join (mapcar #'upcase matches) "_"))
        ('snake (string-join (mapcar #'downcase matches) "_"))
        ('upper-kebab (string-join (mapcar #'upcase matches) "-"))
        ('kebab (string-join (mapcar #'downcase matches) "-"))
        ('pascal (string-join (mapcar #'capitalise-first matches)))
        ('camel (string-join (cons (downcase (car matches)) (mapcar #'capitalise-first (cdr matches))))))))
#+end_src
** Tree Sitter
#+begin_src emacs-lisp
  (require 'treesit)
...
#+begin_src emacs-lisp
  (evil-define-key nil 'global
    ...
    (kbd "M-<right>") (evil-define-command split-right ()
                    (split-window-horizontally)
                    (evil-window-right 1))
    (kbd "M-<left>") (evil-define-command split-left () (split-window-horizontally))
    (kbd "M-<down>") (evil-define-command split-down ()
                    (split-window-vertically)
                    (evil-window-down 1))
    (kbd "M-<up>") (evil-define-command split-up () (split-window-vertically)))
#+end_src
** Text Objects
#+begin_src emacs-lisp
...
** Aardvark
#+begin_src emacs-lisp
  (defhook aardvark-mode-hook
    (lispyville-mode)
    (require 'rainbow-delimiters))
  (ts-install 'aardvark :url "https://github.com/jack-faller/tree-sitter-aardvark.git")
  (straight-use-package '(aardvark-mode :local-repo "~/code/aardvark"))
  (setf aardvark-bracket-face
        (lambda (n)
  	(cl-case (+ 1 (% n 9))
...
#+begin_src emacs-lisp
  ...
  (after-load c-ts-mode
    (smart-tabs-add-language-support c c-ts-mode-hook
  	((treesit-indent . c-ts-mode-indent-offset)
  	 (treesit-indent-region . c-ts-mode-indent-offset)))
