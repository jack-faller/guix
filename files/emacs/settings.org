...
#+begin_src emacs-lisp
  ...
  (defun ts-install (remap mode-name &optional lang-name url-name &rest rest)
    ...
    (let ((orig-mode (symcat mode-name "-mode"))
  	...
  	(with-eval-after-load ts-mode
  	  (push
  	   (cl-list* lang-name (ts-url (or url-name (symbol-name lang-name))) rest)
...
** C#
#+begin_src emacs-lisp
  (ts-install t 'csharp nil "c-sharp")
...
#+begin_src emacs-lisp
  (pkg ocaml-ts-mode)
  (ts-install nil 'ocaml nil nil "master" "grammars/ocaml/src")
  (ts-install nil 'ocaml 'ocaml-interface "ocaml" "master" "grammars/interface/src")
